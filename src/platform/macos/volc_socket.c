...
ssize_t volc_send_msg (int __fd, void* data, size_t size , volc_ip_addr_t* __addr, uint32_t *p_status){
    ...
    do{
        ...
        // char ip[INET_ADDRSTRLEN] = {0};
        // inet_ntop(AF_INET, &addr.sin_addr, ip, sizeof(ip));
        // printf("++++++++++++++++++++volc_send_msg %s af %d %d %d fd %d\n", ip, addr.sin_family,ntohs(addr.sin_port),addr.sin_port,__fd);
    }while(r < 0 && errno == EINTR);
...
// int volc_sockopt_set(int __fd, int __level, int __opt_name,const void *__optval, int __optlen) {
//    return setsockopt(__fd,__level,__opt_name,__optval, (socklen_t)__optlen);
// }

// int volc_sockopt_get(int __fd, int __level, int __opt_name,void * __optval, int * __optlen) {
//     return getsockopt(__fd,__level,__opt_name,__optval, (socklen_t *)__optlen);
// }
